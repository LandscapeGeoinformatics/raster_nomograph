...
class NomographProcessingAlgorithmV2(QgsProcessingAlgorithm):
    """
    ...
    def createInstance(self):
        return NomographProcessingAlgorithmV2()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        feedback.pushInfo('normalising flow length raster into 0-100 (min=0, max=1391.5403)')
        flowlen_band_x = ( flowlen_band - 0 ) / ( flowlen_max_value - 0 ) * 100

        # Update the progress bar
        feedback.setProgress(60)

        feedback.pushInfo('normalising flow accumulation raster into 0-100 (min=0, max=21508)')
        ...
        slope_len_band_x_out = np.nan_to_num( np.where(slope_len_band_x < 3, 5, slope_len_band_x) , copy=False, nan=-1)
